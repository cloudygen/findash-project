pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'findash-app-subodh' // Add your name here
        NEXUS_CRED = 'nexus-auth'
        CONTAINER_NAME = 'findash-app-subodh' // Add your name here
    }
    ...
    stages {
            ...
            steps {
                script {
                    ...
                    docker.withRegistry("http://${NEXUS_REGISTRY}", "${NEXUS_CRED}") {
                        ...
                        // Change the port 9099 to a unique port
                        sh """
                            docker run -d \
                            --name ${CONTAINER_NAME} \
                            -p 9099:8080 \
        ...
        stage('Verify Deployment') {
            // Use the same port in place of 9099 that your set above
            steps {
                script {
                    sh "sleep 5"
                    sh "curl -f http://localhost:9099 || exit 1"
                    echo "Deployment Successful! Access at http://localhost:9099"
                }
            }
        }
    }
